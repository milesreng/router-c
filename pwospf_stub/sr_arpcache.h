...
/* This method handles an ARP request by checking to see if it should be resent or dropped */
void handle_arpreq(struct sr_instance *sr, struct sr_arpreq *req, struct sr_if *iface);

/* This method cnstructs and sends an ARP request for the given IP address on the specified interface */
void sr_send_arp_request(struct sr_instance *sr, uint32_t ip, struct sr_if *iface);

/* This method sends an ICMP Host Unreachable message to the source address of the packet */
void sr_send_icmp_unreachable(struct sr_instance *sr, uint8_t *packet, unsigned int len,
                              const char *iface, uint8_t type, uint8_t code);
